...
export default function Insight5({ isDynamic = true }) {
    ...
    const { data, error, isLoading } = useQuery({
        queryKey: ['tracksPopularityInsight'],
        queryFn: getTracksPopularityInsight,
        staleTime: Infinity,
        select: (data) => {
            let years = []
            ...
            {isDynamic
                ? <div className='h-full flex flex-row gap-5'>
                    <div className='w-9/12'>
                        <ChartWrapper type={CONSTANTS.LINE_CHART} data={data} chartOptions={dynamicChartOptions} />
                    </div>
                ...
                : <ChartWrapper type={CONSTANTS.LINE_CHART} data={data} chartOptions={staticChartOptions} />
            }
        </div>
    )
}